...
void main()
{
	...
	vec3 finalAmbient = light.m_ambient * mat.m_diffuse;

	// Compute the diffuse component
	float diffuseIntensity = max(dot(normalizedNormal, -lightDirViewSpace), 0.0);
	vec3 finalDiffuse = diffuseIntensity * light.m_diffuse * mat.m_diffuse;
	...
	vec3 finalSpecular = specIntensity * light.m_specular * mat.m_diffuse;
